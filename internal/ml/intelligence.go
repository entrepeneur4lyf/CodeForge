...
func (ci *CodeIntelligence) SmartSearch(ctx context.Context, query string, startNodeID string) (*SearchResult, error) {
	...
	if ci.useTD {
		log.Printf("🧠 Using TD Learning for search: %s", query)
...
func (ci *CodeIntelligence) UpdateConfig(config *MLConfig) {
	...
	log.Printf("⚙️ ML configuration updated")
}
...
func (ci *CodeIntelligence) UpdateTDConfig(config *TDConfig) {
	...
	log.Printf("⚙️ TD Learning configuration updated - λ=%.2f", config.Lambda)
}
...
func (ci *CodeIntelligence) SetUseTD(useTD bool) {
	...
	if useTD {
		log.Println("🧠 TD Learning enabled (faster, more efficient)")
	} else {
		log.Println("🧠 TD Learning disabled (using MCTS + Q-Learning)")
	}
}
